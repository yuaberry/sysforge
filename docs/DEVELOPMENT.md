...
./target/release/sysforge doctor      # ver o que o ambiente tem/falta (com comando exato)
...
cd apps/desktop/src-tauri && cargo build    # sysforge-desktop
...
- **Integração** (`daemon/tests/ipc_dev.rs`): sobe o daemon real em dev, conecta o cliente, valida protocolo E o fail-closed (`SF-AUTH-002`).
- **Futuro (Fase 9)**: suite destrutiva real, exclusivamente contra discos QEMU — CI local com `tests/qemu/`.

## Convenções

- Erro novo? Ganha código estável `SF-XXX-NNN` + recomendação acionável.
...
| Tipo | Caminho |
|---|---|
| Log do app/CLI | `~/.local/share/sysforge/logs/sysforge.log` |
| Log do daemon (dev) | `~/.local/share/sysforge/logs/daemon.log` |
| Auditoria (dev) | `~/.local/share/sysforge/logs/audit.jsonl` |
| Auditoria (system) | `/var/lib/sysforge/audit.jsonl` |
| Operações | `/var/lib/sysforge/operations/<id>/operation.json` |
| Socket dev | `$XDG_RUNTIME_DIR/sysforge-osd.dev.sock` |
| Socket system | `/run/sysforge-osd.sock` |
...
sysforge daemon status                # deve responder modo "system"
```

## Repositório

Privado: `github.com/yuaberry/sysforge` (main).
...
- O SYSFORGE instala `~/.config/autostart/polkit-mate-authentication-agent-1.desktop`
